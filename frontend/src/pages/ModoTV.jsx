...
import { AlertTriangle, ArrowLeft, CheckCircle2, Clock3, Factory, Layers3, Maximize, Minimize, MonitorPlay, PackageCheck, Pause, Play, RefreshCw, UsersRound } from "lucide-react"

const INTERVALO_ATUALIZACAO = 30000
const INTERVALO_ROTACAO = 15000
...
export function ModoTV() {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => setPainel((atual) => (atual + 1) % 2), INTERVALO_ROTACAO)
    return () => clearInterval(intervalo)
  }, [rotacaoPausada])
  ...
  return <div className="min-h-screen bg-slate-950 text-white">
    <header className="flex min-h-20 items-center justify-between border-b border-slate-800 bg-slate-900 px-6 py-3">
      <div className="flex items-center gap-4"><div className="rounded-xl bg-blue-600 p-3"><MonitorPlay size={30} /></div><div><h1 className="text-2xl font-black tracking-tight">Produção · Modo TV</h1><p className="text-sm text-slate-400">Painel {painel + 1} de 2 · atualização automática a cada 30 segundos</p></div></div>
    ...
    <main className="p-6">
      ...
      {dados && painel === 1 && <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-blue-500/30 bg-slate-900 p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-2xl font-black text-blue-200">Em execução</h2><p className="text-slate-400">Serviços que já possuem trabalho iniciado</p></div><span className="rounded-full bg-blue-500 px-4 py-2 text-xl font-black">{emExecucao.length}</span></div><div className="space-y-3">{emExecucao.slice(0, 8).map((item) => <LinhaFila key={item.id} item={item} compacta />)}{emExecucao.length === 0 && <p className="py-12 text-center text-xl text-slate-400">Nenhum serviço em execução.</p>}</div></section>
        <section className="rounded-2xl border border-yellow-500/30 bg-slate-900 p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-2xl font-black text-yellow-200">Aguardando início</h2><p className="text-slate-400">Ordenado pelo prazo de entrega do pedido</p></div><span className="rounded-full bg-yellow-400 px-4 py-2 text-xl font-black text-slate-950">{aguardando.length}</span></div><div className="space-y-3">{aguardando.slice(0, 8).map((item) => <LinhaFila key={item.id} item={item} compacta />)}{aguardando.length === 0 && <p className="py-12 text-center text-xl text-slate-400">Nenhum serviço aguardando.</p>}</div></section>
      </div>}
    </main>

    <div className="fixed bottom-0 left-0 flex h-1 w-full">{[0, 1].map((indice) => <button key={indice} type="button" aria-label={`Mostrar painel ${indice + 1}`} onClick={() => setPainel(indice)} className={`h-full flex-1 ${painel === indice ? "bg-blue-500" : "bg-slate-800"}`} />)}</div>
  </div>
}
